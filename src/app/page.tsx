...
import {
  ...
  acceptIncomingOffer, rejectIncomingOffer, counterIncomingOffer, clearLastOffer,
  ...
  setSellPrice, toggleOpenMarketSales, setTransferPricingMode, setTransferPriceMultiplier, buyCompanyShares, sellCompanyShares,
  ...
  buyLandTile, sellLandHolding, buyCompetitorIntel, runEspionage, buyMarketResearch,
} from '@/game/engine';
...
// ============= MAP CANVAS =============
function GameMap({ state, tiles, entities, onTileClick, onBuildingClick, placementMode, landPurchaseMode, camera, onCameraChange }: {
  state: GameState;
  tiles: IsometricTile[][];
  ...
  return (
    <canvas
      ...
      onClick={(e) => {
        ...
        if (clicked && !placementMode && !landPurchaseMode) { onBuildingClick(clicked.id); return; }
        onTileClick(wx, wy);
      }}
      onWheel={(e) => {
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {activePanel && (
          <div className="w-80 bg-gray-800/95 border-r border-gray-700 overflow-y-auto shrink-0">
            ...
            {activePanel === 'products' && <ProductsPanel state={gameState} setState={setGameState} />}
        ...
        <div className="flex-1 relative">
          <GameMap state={gameState} tiles={tiles} entities={entitiesRef}
            ...
            placementMode={placementMode} landPurchaseMode={landPurchaseMode}
            camera={gameState.camera} onCameraChange={handleCameraChange} />
          ...
          <div className="space-y-2">
            {state.incomingOffers.map(offer => (
              <div key={offer.id} className="bg-gray-800/60 rounded p-2 text-xs">
                ...
                {/* Counter offer input */}
                <div className="mt-2 pt-1 border-t border-gray-700 flex gap-1">
                  <input type="number" step={50000} defaultValue={Math.round(offer.fairValue * 1.1)}
                    id={`counter_input_${offer.id}`}
                    className="flex-1 bg-gray-900 border border-gray-600 rounded px-2 py-0.5 text-[10px] text-white outline-none" />
                  <button onClick={() => {
                    const el = document.getElementById(`counter_input_${offer.id}`) as HTMLInputElement;
                    const val = parseFloat(el?.value || '0');
                    if (val > 0) setState(prev => prev ? counterIncomingOffer(prev, offer.id, val) : null);
                  }} className="bg-amber-700 hover:bg-amber-600 px-2 py-0.5 rounded text-[10px] font-bold">
                    Counter
                  </button>
                </div>
              </div>
...
function ProductsPanel({ state, setState }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState | null>> }) {
  const categories = [...new Set(state.products.map(p => p.category))];
  const [expanded, setExpanded] = useState<string | null>(null);
  const player = state.companies.find(c => c.isPlayer);
  const researched = player?.researchedCategories || [];

  return (
    <div className="p-3 space-y-2">
      <h2 className="text-sm font-bold text-emerald-400 border-b border-gray-700 pb-1">📦 Manufacturer&apos;s Guide</h2>
      <p className="text-[10px] text-gray-500">
        Purchase Market Research ($250k/category) to reveal product weights (P/Q/B) and base demand.
      </p>
      {categories.map(cat => {
        const prods = state.products.filter(p => p.category === cat);
        const hasResearch = researched.includes(cat);
          ...
          <div key={cat}>
            ...
            {expanded === cat && (
              <div className="space-y-1 mt-1 ml-2">
                {!hasResearch && (
                  <button onClick={() => setState(prev => prev ? buyMarketResearch(prev, cat) : null)}
                    className="w-full bg-blue-700 hover:bg-blue-600 py-1 rounded text-[10px] mb-2">
                    🔍 Buy Market Research ($250k)
                  </button>
                )}
                {prods.map(p => {
                  ...
                  <div key={p.id} className={`bg-gray-700/30 rounded px-2 py-1 text-xs ${!unlocked ? 'opacity-60' : ''}`}>
                    ...
                    <div className="flex gap-2 text-gray-400 text-[10px]">
                      <span>Q {hasResearch ? p.quality.toFixed(0) : '?'}</span>
                      <span>Brand {hasResearch ? p.brand.toFixed(0) : '?'}</span>
...
function CompaniesPanel({ state }: { state: GameState }) {
  ...
  return (
    <div className="p-3 space-y-2">
      ...
      {sorted.map(c => {
        const hasIntel = c.isPlayer || (c.intelExpiresTick > state.tick && c.playerIntelLevel > 0);
        return (
          <div key={c.id} className="bg-gray-700/50 rounded-lg p-2 text-xs border-l-2" style={{ borderColor: c.color }}>
            ...
            <div className="text-[9px] text-gray-300 mt-0.5">
              {hasIntel ? `${c.personality} · ${c.sectorFocus.replace('_', ' ')}` : `${c.sectorFocus.replace('_', ' ')} · private strategy`}
            </div>
            <div className="grid grid-cols-2 gap-x-3 mt-1 text-gray-400">
              ...
              ) : <span className="text-purple-300">Intel available in Stock Market</span>}
            </div>
            {hasIntel && c.cartelId && <div className="text-[9px] text-amber-400 mt-0.5">🔗 Suspected cartel member</div>}
            {hasIntel && c.predatoryTicks > 0 && <div className="text-[9px] text-red-400 mt-0.5">⚔ Predatory pricing campaign active</div>}
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className="p-3 space-y-3">
      ...
      <div className="bg-gray-700/50 rounded-lg p-2">
        ...
        <div className="text-[10px] font-bold text-gray-400 mt-2 mb-1">Banking · Politics · Environment</div>
        <div className="grid grid-cols-2 gap-1 text-[10px]">
          <div className="bg-gray-800 rounded px-2 py-1">
            <div className="text-gray-500">Bank capital adequacy</div>
            <div className={state.economy.bankCapitalAdequacy < 0.08 ? 'text-red-400' : 'text-green-400'}>
              {(state.economy.bankCapitalAdequacy * 100).toFixed(1)}%
            </div>
          </div>
          <div className="bg-gray-800 rounded px-2 py-1">
            <div className="text-gray-500">Credit tightness</div>
            <div className={state.economy.creditTightness > 0.5 ? 'text-red-400' : 'text-gray-300'}>
              {(state.economy.creditTightness * 100).toFixed(0)}%
            </div>
          </div>
          <div className="bg-gray-800 rounded px-2 py-1">
            <div className="text-gray-500">Loan-loss provisions</div>
            <div>${formatMoney(state.economy.loanLossProvisions)}</div>
          </div>
          <div className="bg-gray-800 rounded px-2 py-1">
            <div className="text-gray-500">Atmospheric CO₂</div>
            <div className={state.economy.co2Stock > 420 ? 'text-red-400' : 'text-gray-300'}>
              {state.economy.co2Stock.toFixed(0)} ppm
            </div>
          </div>
          <div className="bg-gray-800 rounded px-2 py-1">
            <div className="text-gray-500">Ruling party</div>
            <div className="capitalize">{state.politics.rulingParty}</div>
          </div>
          <div className="bg-gray-800 rounded px-2 py-1">
            <div className="text-gray-500">Approval / next election</div>
            <div>{state.politics.approval.toFixed(0)}% · {state.politics.nextElectionYear}</div>
          </div>
        </div>
        <div className="text-[9px] text-gray-500 mt-1">
          Elections every 4 years move tax, antitrust and carbon policy. High debt/GDP with thin bank capital risks a Minsky credit crunch.
        </div>
          ...
          {state.cities.map(city => (
            <div key={city.id} className="text-xs bg-gray-700/30 px-2 py-1 rounded">
              ...
              <div className="flex gap-2 text-[10px] text-gray-500 mt-0.5">
                ...
                <span className={city.pollution > 40 ? 'text-red-400' : 'text-gray-500'}>Poll {city.pollution.toFixed(0)}</span>
                    ...
                    {!intelActive ? (
                      <div className="space-y-1">
                        ...
                        <div className="flex gap-1">
                          ...
                          <button onClick={() => setState(prev => prev ? runEspionage(prev, c.id) : null)}
                            title="60% success. Failure incurs a fine."
                            className="flex-1 bg-red-800 py-0.5 rounded text-[8px]">Espionage $150K</button>
                        </div>
                      </div>
                    ) : (
                      <div className="text-[8px] text-purple-300 space-y-0.5">
                        <div>Intel: {c.personality} · {c.aiStrategy} · risk {(c.riskTolerance * 100).toFixed(0)}% ·
                        horizon {c.planningHorizonMonths}mo · {buildings.length} operating assets</div>
                        {c.espionageCostIntelUntilTick > state.tick && (
                          <div className="text-red-300">Cost floors exposed — you can undercut precisely for {Math.ceil((c.espionageCostIntelUntilTick - state.tick) / (24 * 30))} more months.</div>
                        )}
                      </div>
                    )}
                  </div>
...
function BuildingDetailPanel({ state, building, onClose, setState }: {
  ...
  // Information fog: detailed cost structure of rivals needs a Deep Intel report.
  const intelOk = Boolean(isOwned) || Boolean(company
    && company.playerIntelLevel >= 2 && company.intelExpiresTick > state.tick);
  const costIntel = Boolean(isOwned) || Boolean(company
    && (company.espionageCostIntelUntilTick > state.tick
      || (company.playerIntelLevel >= 2 && company.intelExpiresTick > state.tick)));
  // Operational controls stay locked until construction is complete.
  const complete = building.constructionEndsTick <= state.tick;
  const cfg = BUILDING_CONFIGS[building.type];
      ...
      <div className="p-3 space-y-3">
        <div className="flex justify-between items-start">
          <div>
            ...
            {building.constructionEndsTick > state.tick && (
              <div className="text-[10px] text-amber-400">
                🏗️ Under construction — opens in {Math.ceil((building.constructionEndsTick - state.tick) / 24)} days. No revenue until complete.
              </div>
            )}
            {building.strikeTicks > 0 && (
              <div className="text-[10px] text-red-400">⚠️ Strike: {Math.ceil(building.strikeTicks / 24)} days of lost output</div>
            )}
            {building.unionized && building.strikeTicks <= 0 && (
              <div className="text-[10px] text-amber-300">Unionised workforce (+{(building.unionWagePremium * 100).toFixed(0)}% wages)</div>
            )}
          </div>
        ...
        {building.diseaseTicksRemaining > 0 && (
          <div className="bg-red-900/40 border border-red-600 rounded-lg p-2 text-xs">
            <div className="font-bold text-red-400 mb-1">⚠️ Disease Outbreak Active</div>
            <div className="text-[10px] text-red-200">
              Outbreak active for {Math.ceil(building.diseaseTicksRemaining / 24)} more days. Yield is down 80%. Upgrade veterinary program to prevent future outbreaks.
            </div>
          </div>
        )}

        {venue && building.supply === 0 && complete && (
          <div className="bg-red-900/50 border border-red-600 rounded-lg p-2 text-xs text-center">
            <div className="font-bold text-red-300">⚠️ OUT OF STOCK — $0 REVENUE</div>
            <div className="text-[10px] text-red-200 mt-0.5">
              Missing required ingredients. Connect a supplier or farm in the Supply tab to resume sales.
            </div>
          </div>
        )}
        ...
        {tab === 'overview' && (
          <>
            <div className="grid grid-cols-2 gap-2 text-xs">
              ...
              {isRealEstate && (
                <>
                  ...
                  {isOwned && complete && building.condition < 100 && (
            ...
            <div className="bg-gray-700/50 rounded-lg p-2 text-[10px] text-purple-300">
              Cost structure is private. Purchase a Deep Intel report or run espionage in the Stock Market.
            </div>
            )}
            ...
            {isOwned && complete && !isRealEstate && !isHQ && !isRD && building.targetEmployees > 0 && (
                ...
                {isOwned && complete && (
                  <div className="border-t border-gray-600 pt-2 mt-2 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-1 mt-2">
                      <div className="bg-gray-800 rounded p-1.5 text-[9px]">
                        <div className="text-gray-400">Farm techniques</div>
                        <div className="font-bold">Level {building.farmTechniqueLevel}/5</div>
                        <div className="text-emerald-400">+{building.farmTechniqueLevel * 8}% yield · +{building.farmTechniqueLevel * 5} quality</div>
                        <button
                          onClick={() => setState(prev => prev ? investFarmResearch(prev, building.id) : null)}
                          disabled={building.farmTechniqueLevel >= 5}
                          className="w-full mt-1 bg-purple-700 hover:bg-purple-600 disabled:opacity-40 py-0.5 rounded">
                          Research next technique
                        </button>
                      </div>
                      <div className="bg-gray-800 rounded p-1.5 text-[9px]">
                        <div className="text-gray-400">Equipment</div>
                        <div className="font-bold">Level {building.farmEquipmentLevel}/5</div>
                        <div className="text-emerald-400">+{building.farmEquipmentLevel * 10}% yield · +{building.farmEquipmentLevel * 3} quality</div>
                        <button
                          onClick={() => setState(prev => prev ? upgradeFarmEquipment(prev, building.id) : null)}
                          disabled={building.farmEquipmentLevel >= 5}
                          className="w-full mt-1 bg-blue-700 hover:bg-blue-600 disabled:opacity-40 py-0.5 rounded">
                          Upgrade equipment
                        </button>
                      </div>
                    </div>
              ...
              <div className="space-y-2">
                ...
                {isProducer && (
                  <>
                    ...
                    <div className="bg-gray-700/50 rounded-lg p-2 space-y-1">
                      <div className="text-[10px] font-bold text-gray-300">Transfer Pricing Mode (Intra-Company)</div>
                      <select value={building.transferPricingMode}
                        onChange={e => setState(prev => prev ? setTransferPricingMode(prev, building.id, e.target.value as any) : null)}
                        className="w-full bg-gray-800 border border-gray-600 rounded px-2 py-1 text-[10px] text-white">
                        <option value="cost_basis">Cost Basis (Cost + Logistics) — Shifts Margin to Buyer</option>
                        <option value="custom">Custom Transfer Price Multiplier</option>
                        <option value="market_spot">Market Spot Price</option>
                      </select>
                      {building.transferPricingMode === 'custom' && (
                        <div>
                          <div className="text-[9px] text-gray-400">Transfer Price: {(building.transferPriceMultiplier * 100).toFixed(0)}% of production cost</div>
                          <input type="range" min={0.5} max={2.5} step={0.05} value={building.transferPriceMultiplier}
                            onChange={e => setState(prev => prev ? setTransferPriceMultiplier(prev, building.id, parseFloat(e.target.value)) : null)}
                            className="w-full" />
                        </div>
                      )}
                    </div>
                ...
                <div className="flex gap-2">
                  <button onClick={() => setState(prev => prev ? upgradeBuilding(prev, building.id) : null)}
                    disabled={building.level >= building.maxLevel || !complete}
        ...
        {tab === 'products' && (
          <>
                    ...
                    <div key={item.id} className={`rounded p-2 text-xs ${item.enabled ? 'bg-gray-700/60' : 'bg-gray-800/60 opacity-60'}`}>
                      <div className="flex justify-between items-center">
                        ...
                        <button onClick={() => setState(prev => prev ? toggleMenuItem(prev, building.id, item.id) : null)}
                          disabled={!isOwned || !complete}
                          className={`text-[9px] px-1.5 py-0.5 rounded ${item.enabled ? 'bg-emerald-700' : 'bg-gray-600'} disabled:opacity-40`}>
                          {item.enabled ? 'ON' : 'OFF'}
                        </button>
                      </div>
            ...
            {building.type === 'seaport' && (
              <div className="space-y-2">
                <div className="text-[10px] text-gray-400">
                  State trade terminal. Prices follow the world market, exchange rates, tariffs and local handling.
                  Stock only changes when cargo arrives or a buyer dispatches goods.
                </div>
                <div className="grid grid-cols-2 gap-1 text-[9px]">
                  <div className="bg-gray-800 rounded p-1.5">
                    <div className="text-gray-500">Catalog</div>
                    <div>{building.products.length} products</div>
                  </div>
                  <div className="bg-gray-800 rounded p-1.5">
                    <div className="text-gray-500">Cargo at sea</div>
                    <div>{state.portShipments.filter(shipment => shipment.portBuildingId === building.id).length} shipments</div>
                  </div>
                </div>
                <div className="max-h-64 overflow-y-auto space-y-1">
                  {stocked.map(product => {
                    const stock = building.inventory[product.id] ?? 0;
                    const world = state.globalMarket?.price[product.id] ?? product.productionCost * 0.9;
                    const inbound = state.portShipments.filter(shipment =>
                      shipment.portBuildingId === building.id && shipment.productId === product.id);
                    return (
                      <div key={product.id} className="bg-gray-700/50 rounded px-2 py-1 text-[9px]">
                        <div className="flex justify-between">
                          <span>{product.icon} {product.name}</span>
                          <span className="text-emerald-400">world ${world.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between text-gray-500">
                          <span>Stock {Math.round(stock).toLocaleString()}</span>
                          <span>{inbound.length > 0
                            ? `${inbound.reduce((sum, shipment) => sum + shipment.amount, 0).toLocaleString()} inbound`
                            : 'No inbound cargo'}</span>
                        </div>
                        {inbound.slice(0, 1).map(shipment => (
                          <div key={shipment.id} className="mt-0.5 h-1 bg-gray-800 rounded overflow-hidden" title={`${shipment.origin} · ${shipment.progress.toFixed(0)}%`}>
                            <div className="h-full bg-sky-500" style={{ width: `${shipment.progress}%` }} />
                          </div>
                        ))}
                      </div>
                    );
          ...
          <div className="space-y-2">
            <div className="bg-gray-700/50 rounded-lg p-2 space-y-2">
              <div className="flex items-center justify-between">
                ...
                {isOwned && complete && contractProductIds.length > 0 && (
                  <button
                    onClick={() => setState(prev => prev
              ...
              <div className="text-[9px] text-gray-500">
                Player buildings default to MANUAL: nothing is sourced until you sign a contract below.
                Switch to AUTO only if you want flexible spot-market sourcing. Twelve-month agreements lock price and build loyalty discounts.
              </div>
                    ...
                    {contractOffers.slice(0, 6).map(offer => (
                      <div key={offer.supplierBuildingId} className="bg-gray-800/70 rounded p-1.5 text-[9px]">
                        ...
                        <div className="flex gap-2 text-gray-500">
                          ...
                          <span>Stock {Math.round(offer.availableStock).toLocaleString()}</span>
                        ...
                        <button
                          onClick={() => setState(prev => prev
                            ...
                            : null)}
                          disabled={!complete}
                          className="mt-1 w-full bg-blue-700 hover:bg-blue-600 py-0.5 rounded text-[9px] disabled:opacity-40">
            ...
            {state.pipelineOrders.filter(p => p.toBuildingId === building.id).length > 0 && (
              <div className="bg-gray-700/50 rounded-lg p-2 space-y-1">
                <div className="text-xs font-bold text-gray-300">Inbound Supply Pipeline</div>
                {state.pipelineOrders.filter(p => p.toBuildingId === building.id).map(pOrder => {
                  const prod = state.products.find(p => p.id === pOrder.productId);
                  const isProcessing = pOrder.processingHoursRemaining > 0;
                  return (
                    <div key={pOrder.id} className="bg-gray-800/80 rounded p-1.5 text-[9px]">
                      <div className="flex justify-between font-medium">
                        <span>{prod?.icon} {prod?.name} ({pOrder.amount.toLocaleString()} u)</span>
                        <span className={isProcessing ? 'text-amber-400' : 'text-blue-400'}>
                          {isProcessing ? `Processing (${pOrder.processingHoursRemaining}h)` : `Transit (${pOrder.transitHoursRemaining}h)`}
                        </span>
                      </div>
                      {pOrder.isPerishable && (
                        <div className="text-[8px] text-gray-400 mt-0.5">
                          Cold-chain transit · Total: {pOrder.totalHours}h · Spoilage risk: {(pOrder.spoilageRate * 100).toFixed(0)}%
                        </div>
                      )}
                    </div>
                  );
            ...
            {!costIntel ? (
              <p className="text-xs text-purple-300">
                Supplier contracts are private without a Deep Intel report or successful espionage.
              </p>
                ...
                <div key={link.productId} className="bg-gray-700/60 rounded p-2 text-xs">
                  ...
                  {isOwned && (
                    <button
                      ...
                      disabled={!complete}
                      className="mt-1 w-full bg-red-900/60 hover:bg-red-800 py-0.5 rounded text-[9px] disabled:opacity-40">
        ...
        {!isOwned && building.type !== 'seaport'
          && (building.forSale || building.companyId === 'system') && (
