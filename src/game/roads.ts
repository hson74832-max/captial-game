...
/**
 * The street intersection a highway should tie into when heading toward
 * `(x, y)`. It walks the city's own grid outward in that direction and returns
 * the last node still inside the built-up radius, so the interstate always
 * terminates on a real street node rather than on empty ground.
 */
export function nearestIntersection(city: City, x: number, y: number): { x: number; y: number } {
  const frame = cityRoadFrame(city);
  // Grid node closest to the city centre — guaranteed to exist in the network.
  ...
  // Step outward along the grid toward the target, stopping at the last node
  // that still lies inside the road radius (with a tile of margin).
  let bestX = centreX;
  let bestY = centreY;
...
/**
 * 0..1 proximity to the interstate mesh, measured against the real L-shaped
 * routes rather than straight lines, so the score matches the drawn road.
 */
export function highwayProximity(cities: City[], x: number, y: number): number {
  ...
  for (const [a, b] of highwayEdges(cities)) {
    ...
    for (const [hx, hy] of highwayTiles(exit, entry)) {
      const dist = Math.hypot(x - hx, y - hy);
      if (dist < 0.5) return 1;
      best = Math.max(best, Math.max(0, 1 - dist / 6));
    }
  }
  return best;
}
...
/**
 * The interstate highway between two cities. It runs from the first city's
 * outward-bound intersection to the second city's inbound intersection — the
 * exact segment trucks drive, so the drawn road and the freight route coincide.
 */
export function cityHighway(_state: GameState, fromCity: City, toCity: City): Array<[number, number]> {
  const exit = nearestIntersection(fromCity, toCity.x, toCity.y);
  const entry = nearestIntersection(toCity, fromCity.x, fromCity.y);
  return highwayTiles(exit, entry).map(([x, y]) => [x, y] as [number, number]);
}

/**
 * Every tile the interstate physically occupies between two endpoints.
 *
 * The route is an L-shape: it runs along one axis, turns once, then runs along
 * the other — matching how the isometric renderer draws tiles and guaranteeing
 * an unbroken, orthogonally-connected chain with no diagonal gaps.
 */
export function highwayTiles(
  ...
  // Turn on whichever axis is longer first, so the dog-leg looks natural.
  ...
  for (let x = startX; x !== cornerX; x += stepX1) push(x, startY);
  const stepY1 = Math.sign(cornerY - startY);
  for (let y = startY; y !== cornerY; y += stepY1) push(cornerX === startX ? startX : cornerX, y);

  push(cornerX, cornerY);

  const stepX2 = Math.sign(endX - cornerX);
  for (let x = cornerX; x !== endX; x += stepX2) push(x, cornerY);
  const stepY2 = Math.sign(endY - cornerY);
  for (let y = cornerY; y !== endY; y += stepY2) push(endX, y);
...
export function buildFreightPolyline(
  ...
  // Leave via the intersection closest to the destination, enter via the
  // closest to the origin — the same nodes the drawn highway connects.
  ...
  const highway = cityHighway(state, fromCity, toCity);
  const localIn = findRoadPath(toNetwork, destEntry.id, destEnd.id);

  const segments = [...localOut, ...highway.slice(1), ...localIn.slice(1)];
  return segments.length > 0 ? segments : [[supplier.x, supplier.y], [buyer.x, buyer.y]];
}
