...
export function updateEntities(entities: MovingEntity[], state: GameState, deltaScale = 1): void {
  ...
  // Spawn a truck for any freight route without one. Trucks are cosmetic —
  // routes are the actual simulation — so cap the fleet to keep rendering cheap.
  const existingIds = new Set(entities.map(e => e.id));
  const truckCount = entities.filter(e => e.type === 'freight_truck').length;
