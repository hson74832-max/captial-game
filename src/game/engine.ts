...
function generateCompanies(rand: () => number, playerName: string, playerColor: string = '#10b981'): Company[] {
  ...
  for (let i = 0; i < numAI; i++) {
    ...
    ai.push({
      ...
      playerIntelLevel: 0, intelExpiresTick: 0,
      lossCarryforward: 0, taxesPaidYTD: 0, pretaxProfitYTD: 0, taxesPaidLastYear: 0,
...
function simulateBuildings(state: GameState, index: StateIndex): void {
  ...
  for (const building of state.buildings) {
    ...
    if ((building.type === 'farm' || building.type === 'mine' || building.type === 'factory') && product) {
      ...
      const spotPrice = product.currentPrice * 0.72 * building.sellPriceMultiplier * farmPriceMul;
...
function simulateCompanies(state: GameState, index: StateIndex): void {
  for (const company of state.companies) {
    ...
    const landValue = state.landHoldings
      .filter(holding => holding.ownerId === company.id)
  ...
  for (const company of state.companies) {
    ...
    if (!bleeding && company.acumen > 0.4 && Math.random() < 0.25) {
      ...
      if (bargains.length > 0) {
        ...
        company.cash -= target.askingPrice;
        if (seller && !seller.isPlayer) seller.cash += target.askingPrice;
        target.companyId = company.id;
        target.forSale = false;
        target.askingPrice = 0;
        target.purchasePrice = target.askingPrice;
    ...
    if (company.cash > 300_000 && !bleeding && Math.random() < buildChance) {
      ...
      if (best && best.score > investmentThreshold && best.type !== 'mine') {
        ...
        if (company.cash >= cost * 1.5) {
          ...
          company.buildings.push(building.id);
          if (parcel) parcel.developedBuildingId = building.id;
          if (Math.random() < 0.55) {
  ...
  for (const productId of [...new Set(needed)]) {
    ...
    const quotes = supplierPool
      ...
      .map(sup => {
        ...
        // Port list price tracks the DYNAMIC world market price (which
        // moves with net-export flows), not a hard-coded ×1.28 markup.
        const worldPrice = state.globalMarket?.price[productId] ?? inputProduct.productionCost * 0.9;
        const listPrice = sup.companyId === 'system'
          ? worldPrice * 1.12 * importMul  // port markup for handling, customs, insurance
          : inputProduct.productionCost * (0.92 + sup.trainingLevel * 0.025);
...
function simulateMonthly(state: GameState): void {
  ...
  // Prices move with cumulative net export flows: net imports drag world
  // prices down (arbitrageurs), net exports pull them up. Prices also
  // mean-revert toward the underlying production cost.
  if (state.globalMarket) {
    for (const product of state.products) {
      const gm = state.globalMarket;
      ...
      // Elasticity: 1% price shift per 5% of monthly production volume in net flow.
      const monthlyProdEstimate = Math.max(1000, product.marketDemand * 100);
      const netFlowPct = (gm.netExport[product.id] ?? 0) / monthlyProdEstimate;
      const flowShift = Math.max(-0.1, Math.min(0.1, netFlowPct * 0.2));
      const reversion = (targetPrice - current) * 0.08;
      gm.price[product.id] = Math.max(product.productionCost * 0.4,
        Math.min(product.productionCost * 2.5, current + reversion + current * flowShift));
      // Decay the net export tally toward zero (30-day rolling).
      gm.netExport[product.id] = (gm.netExport[product.id] ?? 0) * 0.7;
    }
  }
...
export function getLandPurchasePrice(
  ...
  return Math.max(10_000, tile.landValue * 2_400 * wageFactor * populationFactor
    * trafficFactor * zoningFactor * cbdFactor * developmentFactor * resourceFactor);
}
...
export function placeBuilding(
  ...
  if (ownedLand) ownedLand.developedBuildingId = building.id;
