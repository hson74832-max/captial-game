import type {
  ...
  ForwardGuidance, ZoneType, MovingEntity, MenuItem, SupplierLink, SupplyContractOffer,
} from './types';
import {
...
function generateInitialBuildings(cities: City[], companies: Company[], products: Product[], year: number): Building[] {
  ...
  for (const [i, city] of portCities.entries()) {
    ...
    port.inventory = Object.fromEntries(stock.map(p => [p.id, 300000]));
    port.inventoryCapacity = 2_000_000;
...
export function createNewGame(seed: number = 1337, playerName: string = 'Your Corporation'): GameState {
  ...
  const state: GameState = {
    ...
    economy: {
      ...
      carbonTaxPerUnit: 0, dividendTaxRate: 15, bankCredit: 0, nominalGdp: 0,
  ...
  for (const building of state.buildings) {
    ...
    if ((building.type === 'farm' || building.type === 'mine' || building.type === 'factory') && product) {
      ...
      const productionUnits = building.capacity / 24 * efficiency * seasonal * farmGrowth * farmWeather * inputReadiness
        * reserveFactor * rush.output * (0.7 + building.effectiveTraining / 15) * staffingFactor;
      ...
      const marketAbsorb = (building.internalSale || !building.openMarketSales)
        ...
        : (product.marketDemand / 100) * (building.capacity / 12)
          * (0.7 + city.population / 600_000) * priceElasticity;
      ...
      const spotPrice = product.currentPrice * 0.72 * building.sellPriceMultiplier;
    ...
    // Retail
      ...
      for (const line of stocked) {
        ...
        const desired = retailDemand(city, line, building, localTraffic, confidence,
          isDaytime, index.buildingsByCity);
...
function simulateCompanies(state: GameState, index: StateIndex): void {
  for (const company of state.companies) {
    ...
    if (company.isPlayer) {
      state.player.netWorth = state.player.cash + company.cash + bookValue
        + playerPortfolioValue(state) - company.debt;
    }
  }
...
function simulateAICompanies(state: GameState): void {
  for (const company of state.companies) {
    ...
    // Herding: euphoric boards overexpand (the bubble), panicked boards freeze.
    const sentimentGate = Math.max(0.25, Math.min(0.9, company.sentiment / 1.5));
    // Novices are reactive: build on hope, close on panic, price wherever.
    const gate = Math.max(0.35, Math.min(0.95, (appetite - 0.25) / Math.max(0.4, sentimentGate * (0.7 + company.acumen * 0.6))));
    ...
    if (company.cash > 300_000 && !bleeding && Math.random() > gate) {
      ...
      if (best && best.score > 0.25 && best.type !== 'mine') {
        ...
        if (company.cash >= cost * 1.15) {
          const building = createBuilding(best.type, company.id, best.city, state.products, undefined, undefined, state.cities, state.year);
          building.constructionCost = cost;
          company.cash -= cost;
  ...
  for (const productId of [...new Set(needed)]) {
    ...
    const quotes = supplierPool
      ...
      .map(sup => {
        ...
        const listPrice = inputProduct.productionCost
          * (sup.companyId === 'system' ? 1.28 : 0.92 + sup.trainingLevel * 0.025)
          * importMul;
        const pricePerUnit = listPrice * (1 - loyaltyDiscount) * volumeDiscount;
        const quality = sup.type === 'mine'
          ? inputProduct.quality
          : Math.min(100, inputProduct.quality + sup.trainingLevel * 2.2);
        const stock = sup.companyId === 'system' ? Number.POSITIVE_INFINITY : (sup.inventory[productId] ?? 0);
    ...
    // Do not place a duplicate order while one is already in transit.
    const alreadyInTransit = state.freight.some(route =>
      route.toBuildingId === building.id && route.good === productId);
    if (alreadyInTransit) continue;

    const wanted = Math.min(building.inventoryCapacity,
      Math.max(contract.minimumOrder, expectedConsumptionPerDay * 30 - onHand));
    const availableAtSupplier = selected.sup.companyId === 'system'
      ? wanted
      : Math.min(wanted, selected.sup.inventory[productId] || 0);
    ...
    state.freight.push({
      id: generateId(), fromBuildingId: selected.sup.id, toBuildingId: building.id,
      good: productId, amount: availableAtSupplier, progress: 0,
      distance: Math.max(1, Math.hypot(selected.sup.x - building.x, selected.sup.y - building.y)),
...
export function simulateTick(state: GameState): GameState {
  ...
  if (s.tick % 24 === 0) refreshSupplyNetworks(s);
...
export function placeBuilding(
  ...
  let cost = tiles ? materials + labour + land : cfg.cost * city.landCostMultiplier;
...
export function demolishBuilding(state: GameState, buildingId: string): GameState {
  ...
  // Scrap value: 20% of depreciated construction plus land.
  const salePrice = building.constructionCost * 0.2 * (building.condition / 100)
    + building.landValue * 0.95;

  // Capital gains tax applies if there's somehow a gain, but demolition is usually a loss.
  const basis = building.purchasePrice > 0 ? building.purchasePrice : salePrice;
  ...
  addNotification(state, {
    id: generateId(),
    message: `Demolished ${building.name} and recovered $${formatMoney(salePrice)} in land and scrap value.`,
    type: 'info', tick: state.tick,
  });
...
export function setRentMultiplier(state: GameState, buildingId: string, multiplier: number): GameState {
  ...
  // ── Annual rent adjustment rule ──
  // Asking rent can only be changed once per year per building. Existing
  // tenants keep the old rate until their lease expires (~3 months after
  // the change takes effect).
  const YEAR_TICKS = 24 * 30 * 12;
  if (building.rentLastAdjustedTick > 0 && state.tick - building.rentLastAdjustedTick < YEAR_TICKS) {
    const monthsLeft = Math.ceil((YEAR_TICKS - (state.tick - building.rentLastAdjustedTick)) / (24 * 30));
    addNotification(state, {
      id: generateId(),
      message: `Rent was adjusted this year — next change available in ${monthsLeft} month(s).`,
      type: 'warning', tick: state.tick,
    });
    return state;
  }
  ...
  addNotification(state, {
    id: generateId(),
    message: `Rent adjusted to ${(multiplier * 100).toFixed(0)}% of market. Next change in 12 months.`,
    type: 'info', tick: state.tick,
  });
  return { ...state };
}
...
export function takeLoan(state: GameState, amount: number, termMonths: number): GameState {
  ...
  // Borrowing capacity is 60% of NET assets. TotalAssets includes cash, so
  // the old formula (0.7 x totalAssets - debt) let the player pyramid: each
  // loan added cash, which raised totalAssets, which raised capacity again —
  // converging on 233% of starting assets in debt.
  ...
  // Leverage risk premium: the deeper the debt, the higher the rate.
...
export function getSupplyContractOffers(
  ...
  return state.buildings
    ...
    .map(supplier => {
      ...
      const pricePerUnit = product.productionCost
        * (supplier.companyId === 'system' ? 1.28 : 0.92 + supplier.trainingLevel * 0.025)
        * importMul * (1 - loyaltyDiscount);
      ...
      return {
        ...
        availableStock: supplier.companyId === 'system'
          ? Number.POSITIVE_INFINITY : (supplier.inventory[productId] ?? 0),
        loyaltyDiscount,
      };
    })
