...
export function collectPropertyTax(state: GameState, index: StateIndex) {
  ...
  for (const company of state.companies) {
    ...
    // Vacant land is taxed directly; developed land is already included in
    // its building's assessed value above.
    const vacantLand = state.landHoldings.filter(holding =>
      holding.ownerId === company.id && !holding.developedBuildingId);
    const landBill = vacantLand.reduce((sum, holding) =>
      sum + holding.currentValue * holding.propertyTaxRate / 12, 0);
    for (const holding of vacantLand) {
      if (holding.cityId) receiptsByCity.set(holding.cityId,
        (receiptsByCity.get(holding.cityId) ?? 0)
          + holding.currentValue * holding.propertyTaxRate / 12);
    }
    if (landBill > 0) company.cash -= landBill;
    bill += landBill;
    if (bill <= 0) continue;
    // Building tax cash impact accrues hourly; vacant-land tax is charged above.
...
export function replenishSeaports(state: GameState) {
  ...
  for (const port of state.buildings) {
    ...
    for (const pid of port.products) {
      ...
      // Target stock varies by product value and demand; bulk materials travel
      // in larger lots, electronics in smaller containers.
      ...
      const transitHours = Math.round(72 + Math.random() * 288); // 3–15 days
      state.portShipments.push({
        id: `portship_${Math.random().toString(36).slice(2, 10)}`,
        portBuildingId: port.id, productId: pid, amount, progress: 0,
        transitHours,
