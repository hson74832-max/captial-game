...
export function updateSocialProof(building: Building, unitsSold: number, capacityPerHour: number) {
  ...
  // FOMO is convex: a product that is already hot accelerates far faster than
  // one that is merely doing well, producing genuine viral runs.
  const viral = performance > 0.75 ? 0.035 : performance > 0.5 ? 0.014 : 0.008;
  building.socialProof += (performance - building.socialProof) * viral;
 ...
 *
 * Consumers weigh a price *rise* roughly 2.25× more heavily than an equivalent
 * price *cut*. A 10% increase therefore drives away far more custom than a 10%
 * discount attracts — which is why real retailers dread raising list prices and
 * lean on promotions instead.
 */
export function lossAversionMultiplier(currentPrice: number, referencePrice: number): number {
  if (referencePrice <= 0) return 1;
  const delta = (currentPrice - referencePrice) / referencePrice;
  const LAMBDA = 2.25;   // loss-aversion coefficient
  const ALPHA = 0.88;    // diminishing sensitivity exponent
  // Gains (a discount) feel good but with diminishing returns; losses (a rise)
  // are amplified by lambda.
  const value = delta < 0
    ? Math.pow(-delta, ALPHA)              // perceived as a gain
    : -LAMBDA * Math.pow(delta, ALPHA);    // perceived as a loss
  return Math.max(0.25, Math.min(1.6, 1 + value * 0.55));
}

/**
 * Herding / bandwagon demand. Once a product crosses a visibility threshold in
 * a city, peer effects compound: people buy it because other people are buying
 * it, independent of price or quality.
 */
export function bandwagonMultiplier(socialProof: number, brandAwareness: number): number {
  const buzz = socialProof * 0.7 + (brandAwareness / 100) * 0.3;
  // Below the threshold nothing happens; above it, demand snowballs.
  if (buzz < 0.35) return 1;
  return 1 + Math.pow((buzz - 0.35) / 0.65, 1.6) * 0.85;
}
