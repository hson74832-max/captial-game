...
function drawBuilding(ctx: CanvasRenderingContext2D, building: Building, state: GameState, canvasW: number, canvasH: number, time: number) {
  ...
  const footprintJitter = building.type === 'apartment' ? 0.85 + (idHash % 35) / 100 : 1;
  const heightJitter = building.type === 'apartment' ? 6 + (idHash % 5) * 4 : 0;
  const baseW = (cfg.w * 0.5) * TILE_W * zoom * footprintJitter;
  const baseH = (cfg.h * 0.5) * TILE_H * zoom * footprintJitter;
  const height = (12 + building.level * 5 + heightJitter) * zoom;
  ...
  if (isPlayerBuilding) {
    ctx.save();
    ctx.beginPath();
    ctx.moveTo(sx, sy - baseH / 2 - 1.5 * zoom);
    ctx.lineTo(sx + baseW / 2 + 2.5 * zoom, sy - 1.5 * zoom);
    ctx.lineTo(sx, sy + baseH / 2 + 1.5 * zoom);
    ctx.lineTo(sx - baseW / 2 - 2.5 * zoom, sy - 1.5 * zoom);
    ctx.closePath();
    ctx.strokeStyle = 'rgba(255,255,255,0.85)';
    ctx.lineWidth = 1.4 * Math.min(1.4, zoom);
    ctx.stroke();
    ctx.restore();
  }
