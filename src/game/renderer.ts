...
import { cityRoadFrame, CITY_ROAD_RADIUS, getRoadNetwork } from './roads';
...
/** Stable per-instance hash for colour jitter. */
...
export function renderGame(
  ...
  // Pre-compute which tiles are covered by a building footprint so the terrain
  // pass can skip decorative flora (trees, mountain peaks) that would otherwise
  // poke through walls and roofs.
  const occupied = new Set<string>();
  for (const building of state.buildings) {
    const halfW = Math.max(0, Math.floor(building.width / 2));
    const halfH = Math.max(0, Math.floor(building.height / 2));
    for (let ty = building.y - halfH - 1; ty <= building.y + halfH + 1; ty++) {
      for (let tx = building.x - halfW - 1; tx <= building.x + halfW + 1; tx++) {
        occupied.add(`${Math.round(tx)},${Math.round(ty)}`);
      }
    }
  }

  // Draw terrain
  drawTerrain(ctx, tiles, state, minX, maxX, minY, maxY, w, h, time, occupied);
  ...
  // Delivery service area for the selected venue, drawn as an isometric
  // ellipse so the player can see exactly which households are reachable.
  const selected = state.selectedBuilding
    ? state.buildings.find(b => b.id === state.selectedBuilding)
    : null;
  if (selected?.delivery.enabled) {
    drawDeliveryRadius(ctx, selected, state, w, h, time);
  }
...
function drawTerrain(ctx: CanvasRenderingContext2D, tiles: IsometricTile[][], state: GameState,
  ...
  for (let y = minY; y <= maxY; y++) {
    ...
    for (let x = minX; x <= maxX; x++) {
      ...
      const covered = occupied?.has(`${x},${y}`) ?? false;
      drawTile(ctx, sx, sy, tile, state, z, time, x === Math.floor(state.camera.x) && y === Math.floor(state.camera.y), covered);
    }
  }
}

function drawTile(ctx: CanvasRenderingContext2D, sx: number, sy: number, tile: IsometricTile,
                  state: GameState, zoom: number, time: number, _isCenter: boolean, covered = false) {
  ...
  // Tile edges rely on flat-shading variation between adjacent tiles rather
  // than outline strokes — a clean, unlined isometric look.
  // Water shimmer
  ...
  // Terrain flora is suppressed on tiles covered by a building footprint —
  // otherwise a forest tree pokes through the wall of an apartment.
  const canDecorate = !covered;

  // Forest: trees
  if (canDecorate && tile.type === 'forest' && zoom > 0.5) {
    drawTree(ctx, sx, sy - th * 0.5, zoom, tile.variant, time);
  }

  // Mountain: snow cap or peak
  if (canDecorate && tile.type === 'mountain' && zoom > 0.5) {
    drawMountainPeak(ctx, sx, sy, zoom, tile.variant);
  }

  // Snow
  if (canDecorate && tile.type === 'snow' && zoom > 0.5) {
  ...
  // Hills: small bumps
  if (canDecorate && tile.type === 'hills' && zoom > 0.5) {
  ...
  // Interstate highway — takes priority over biome and city roads. Over water
  // it renders as a bridge with visible deck edges.
  if (tile.highway) {
    const isBridge = tile.type === 'water';
    ctx.fillStyle = isBridge ? '#4a4a50' : '#333338';
    ctx.beginPath();
    ctx.moveTo(sx, sy - th / 2);
    ctx.lineTo(sx + tw / 2, sy);
    ctx.lineTo(sx, sy + th / 2);
    ...
    ctx.closePath();
    ctx.fill();
    if (isBridge && zoom > 0.55) {
      // Bridge deck edges
      ctx.fillStyle = '#5a5a62';
      ctx.beginPath();
      ctx.moveTo(sx - tw / 2, sy);
      ctx.lineTo(sx - tw / 2, sy + th * 0.18);
      ctx.lineTo(sx, sy + th / 2 + th * 0.18);
      ctx.lineTo(sx, sy + th / 2);
      ctx.closePath();
      ctx.fill();
      ctx.beginPath();
    ...
    ctx.fillRect(sx - 0.7 * zoom, sy - th * 0.28, 0.9 * zoom, th * 0.56);
    ctx.fillStyle = '#e8c547';
    ctx.fillRect(sx + 0.2 * zoom, sy - th * 0.28, 0.9 * zoom, th * 0.56);
  }

  // City roads (grid pattern — spacing varies per city)
  if (!tile.highway && tile.cityId && tile.type !== 'water') {
  ...
  // A visible kerbside strip on every generated bay tile, so the "cannot build
  // on parking" rule is legible on the map instead of hidden.
  if (tile.cityId && !tile.highway && zoom > 0.5) {
    const city = state.cities.find(c => c.id === tile.cityId);
    if (city) {
      const distFromCenter = Math.hypot(tile.x - city.x, tile.y - city.y);
      if (distFromCenter < CITY_ROAD_RADIUS) {
        const frame = cityRoadFrame(city);
        // The tile qualifies as a bay when it sits exactly one tile off a
        // street node (matches the road-network parking placement).
        const nearestX = frame.originX + Math.round((tile.x - frame.originX) / frame.spacingX) * frame.spacingX;
        const nearestY = frame.originY + Math.round((tile.y - frame.originY) / frame.spacingY) * frame.spacingY;
        const offX = tile.x - nearestX;
        const offY = tile.y - nearestY;
        const isBayCandidate = (Math.abs(offX) === 1 && offY === 0) || (Math.abs(offY) === 1 && offX === 0);
        if (isBayCandidate) {
          // Only render if a road-network bay actually exists here.
          const network = getRoadNetwork(state, city);
          const isBay = network.parking.some(slot => Math.round(slot.x) === tile.x && Math.round(slot.y) === tile.y);
          if (isBay) {
            ctx.fillStyle = '#3f3f45';
            const alongX = Math.abs(offY) === 1;
            if (alongX) {
              // Bay parallel to a horizontal street
              ctx.fillRect(sx - tw * 0.3, sy - th * 0.06, tw * 0.6, th * 0.12);
            } else {
              ctx.fillRect(sx - tw * 0.06, sy - th * 0.3, tw * 0.12, th * 0.6);
            }
            // Painted P marker
...
/**
 * Delivery service area. In isometric space a circle of world-radius r maps to
 * an ellipse, so we trace the true tile-space circle and project each sample —
 * the outline therefore matches the tiles the couriers can actually reach.
 * Households inside the zone are highlighted so the catchment is legible.
 */
function drawDeliveryRadius(
  ctx: CanvasRenderingContext2D,
  building: Building,
  state: GameState,
  canvasW: number,
  ...
  const radius = building.delivery.radius;
  if (radius <= 0) return;

  const zoom = state.camera.zoom;
  const pulse = 0.55 + Math.sin(time * 2) * 0.18;
  const inHouse = building.delivery.mode === 'in_house';
  const stroke = inHouse ? '56, 189, 248' : '167, 139, 250'; // sky vs violet
  ...
  for (let i = 0; i <= SEGMENTS; i++) {
    ...
    const wx = building.x + Math.cos(angle) * radius;
    const wy = building.y + Math.sin(angle) * radius;
    const [px, py] = worldToScreen(wx, wy, state.camera, canvasW, canvasH);
    if (i === 0) ctx.moveTo(px, py);
    else ctx.lineTo(px, py);
  }
  ctx.closePath();
  ctx.fillStyle = `rgba(${stroke}, 0.10)`;
  ctx.fill();
  ctx.strokeStyle = `rgba(${stroke}, ${pulse})`;
  ctx.lineWidth = 2;
  ctx.setLineDash([8, 6]);
  ...
  // Highlight reachable households — these are the actual demand pool.
  let reachable = 0;
  for (const other of state.buildings) {
    if (other.type !== 'apartment' || other.cityId !== building.cityId) continue;
    if (Math.hypot(other.x - building.x, other.y - building.y) > radius) continue;
    reachable++;
    const [hx, hy] = worldToScreen(other.x, other.y, state.camera, canvasW, canvasH);
    ctx.beginPath();
    ctx.arc(hx, hy, 3.2 * zoom, 0, Math.PI * 2);
    ctx.fillStyle = `rgba(${stroke}, 0.85)`;
    ctx.fill();
  }

  // Legend pinned above the venue.
  const [lx, ly] = worldToScreen(building.x, building.y - radius, state.camera, canvasW, canvasH);
  const label = `${inHouse ? 'Own fleet' : 'Marketplace'} · ${radius} tiles · ${reachable} blocks · ${building.delivery.avgDeliveryMinutes.toFixed(0)} min`;
  ctx.font = `bold ${Math.max(9, 10 * zoom)}px sans-serif`;
  ctx.textAlign = 'center';
  const metrics = ctx.measureText(label);
  ctx.fillStyle = 'rgba(2, 6, 23, 0.85)';
  ctx.beginPath();
  ctx.roundRect(lx - metrics.width / 2 - 7, ly - 22, metrics.width + 14, 17, 5);
  ctx.fill();
  ctx.fillStyle = building.delivery.avgDeliveryMinutes > 45
    ? '#f87171'
    : building.delivery.avgDeliveryMinutes > 30 ? '#fbbf24' : '#4ade80';
...
// Residential palettes — a district should look like a district, not a
// photocopy. Four brick/plaster schemes rotate by sprite, with per-instance
// height and footprint jitter on top.
const APARTMENT_SCHEMES: Array<{ wall: string, roof: string, accent: string }> = [
  { wall: '#e9d5ff', roof: '#c084fc', accent: '#7c3aed' }, // lilac
  { wall: '#fde68a', roof: '#b45309', accent: '#78350f' }, // terracotta plaster
  { wall: '#cbd5e1', roof: '#475569', accent: '#1e293b' }, // slate block
  { wall: '#d1fae5', roof: '#059669', accent: '#065f46' }, // sage
  { wall: '#fecaca', roof: '#991b1b', accent: '#7f1d1d' }, // red brick
  { wall: '#e2e8f0', roof: '#64748b', accent: '#334155' }, // white stucco
];

const COMMERCIAL_SCHEMES: Array<{ wall: string, roof: string, accent: string }> = [
  { wall: '#cbd5e1', roof: '#94a3b8', accent: '#475569' },
  { wall: '#bfdbfe', roof: '#1d4ed8', accent: '#1e3a8a' },
  { wall: '#e0e7ff', roof: '#4338ca', accent: '#312e81' },
  { wall: '#d1d5db', roof: '#111827', accent: '#374151' },
];
...
function drawBuilding(ctx: CanvasRenderingContext2D, building: Building, state: GameState, canvasW: number, canvasH: number, time: number) {
  ...
  const zoom = state.camera.zoom;
  const idHash = hashId(building.id);

  // Residential/commercial blocks draw from a scheme library so neighbouring
  // buildings differ in material and silhouette.
  let palette = getBuildingPalette(building.type);
  if (building.type === 'apartment') palette = APARTMENT_SCHEMES[(building.sprite + idHash) % APARTMENT_SCHEMES.length];
  else if (building.type === 'commercial') palette = COMMERCIAL_SCHEMES[(building.sprite + idHash) % COMMERCIAL_SCHEMES.length];
  ...
  // Per-instance silhouette jitter: residential towers range from squat to
  // slender so the skyline has variety even within one city.
  const footprintJitter = building.type === 'apartment' ? 0.94 + (idHash % 11) / 100 : 1;
  const heightJitter = building.type === 'apartment' ? 4 + (idHash % 5) * 3 : 0;

  // ── Isometric footprint with a plot setback ──────────────────────────────
  // A w×h parcel projects to a diamond whose full width is (w + h)·TILE_W/2.
  // Buildings do not render to the parcel boundary — real structures sit back
  // from the lot line, leaving kerb, service access and yard. PLOT_COVERAGE is
  // that setback, and it also stops neighbouring lots visually colliding.
  const PLOT_COVERAGE = 0.62;
  const parcelW = (cfg.w + cfg.h) * (TILE_W / 2) * zoom;
  const parcelH = (cfg.w + cfg.h) * (TILE_H / 2) * zoom;
  const baseW = parcelW * PLOT_COVERAGE * footprintJitter;
  const baseH = parcelH * PLOT_COVERAGE * footprintJitter;
  // Storey height scales with the parcel so a large plot does not produce a
  // squat slab, but stays bounded so towers never dwarf the district.
  const storeyScale = Math.min(1.5, 0.75 + (cfg.w + cfg.h) * 0.12);
  const height = (10 + building.level * 4.5 + heightJitter) * zoom * storeyScale;
  ...
  // Per-instance tint so adjacent same-type buildings still read as separate
  // masses without outline strokes.
  const h = hashId(building.id);
  const wallShade = 0.95 + (h % 9) * 0.009; // ±~4% lightness jitter
  ...
  ctx.closePath();
  ctx.fillStyle = darken(palette.wall, wallShade);
  ctx.fill();
  ...
  ctx.lineTo(sx, sy + baseH / 2 - height);
  ctx.closePath();
  ctx.fillStyle = darken(palette.wall, 0.78 * wallShade);
  ctx.fill();
  ...
  // ── Ownership marking ────────────────────────────────────────────────────
  // Every firm carries a pennant on its roofline in its house colour, so the
  // map reads as a patchwork of corporate territory. The player's own
  // buildings additionally get a glowing base ring — unmistakable at a glance.
  const isPlayerBuilding = building.companyId === state.companies.find(c => c.isPlayer)?.id;
  drawOwnershipFlag(ctx, sx - baseW / 2, sy - height, companyColor, isPlayerBuilding, zoom);

  if (isPlayerBuilding) {
    // Ground-level footprint outline in the house colour — an at-a-glance cue
    // that this asset belongs to the player, even against rival buildings that
    // otherwise use the same silhouette.
    ctx.save();
    ctx.beginPath();
    ctx.moveTo(sx, sy - baseH / 2 - 1 * zoom);
    ctx.lineTo(sx + baseW / 2 + 2 * zoom, sy - 1 * zoom);
    ctx.lineTo(sx, sy + baseH / 2 + 1 * zoom);
    ctx.lineTo(sx - baseW / 2 - 2 * zoom, sy - 1 * zoom);
    ctx.closePath();
    ctx.strokeStyle = 'rgba(16,185,129,0.95)';
    ctx.lineWidth = 2 * Math.min(1.6, zoom);
    ctx.stroke();

    // A soft outer glow so the outline reads even at low zoom.
    ctx.strokeStyle = 'rgba(16,185,129,0.35)';
    ctx.lineWidth = 4 * Math.min(1.6, zoom);
    ctx.stroke();
    ctx.restore();
  }
  ...
  // ── Labels ────────────────────────────────────────────────────────────────
  // Only the selected building, or buildings at high zoom, are labelled. This
  // stops the map turning into a wall of overlapping text at city scale.
  const showLabel = isSelected || zoom > 1.25;
  if (showLabel) {
    const fontSize = Math.max(8, 9 * zoom);
    ctx.font = `bold ${fontSize}px sans-serif`;
    ctx.textAlign = 'center';
    const labelY = sy - height - 6 * zoom;
    const text = building.name;
    const metrics = ctx.measureText(text);
    // Solid plate behind the text so it stays legible over any roof colour.
    ctx.fillStyle = 'rgba(2, 6, 23, 0.78)';
    ctx.beginPath();
    ctx.roundRect(sx - metrics.width / 2 - 5, labelY - fontSize + 1, metrics.width + 10, fontSize + 5, 4);
    ctx.fill();
    ctx.fillStyle = '#fff';
    ctx.fillText(text, sx, labelY + 1);
  }

  // Rent tag sits below the base diamond and only for the selected asset, so
  // adjacent blocks never stack their tags on top of one another.
  if (isSelected && (building.type === 'apartment' || building.type === 'commercial') && building.rentPerUnit > 0) {
    ...
    const tagY = sy + baseH / 2 + 8 * zoom;
    ctx.fillStyle = 'rgba(2, 6, 23, 0.82)';
    ctx.beginPath();
    ctx.roundRect(sx - rentW / 2 - 5, tagY - 8, rentW + 10, 13, 3);
    ctx.fill();
    ctx.fillStyle = building.occupancy > 70 ? '#4ade80' : '#fbbf24';
    ctx.fillText(rentText, sx, tagY + 1.5);
  }
}
...
/**
 * Pennant on the roofline, in the owner's house colour. Player buildings get
 * a larger pennant with a bright mast cap so "mine vs. theirs" reads instantly
 * at map scale.
 */
function drawOwnershipFlag(ctx: CanvasRenderingContext2D, x: number, y: number, color: string, isPlayer: boolean, zoom: number) {
  if (zoom < 0.45) return;
  const h = (isPlayer ? 10 : 7) * zoom;
  const w = (isPlayer ? 7 : 5) * zoom;

  // Mast
  ctx.fillStyle = isPlayer ? '#f8fafc' : 'rgba(255,255,255,0.75)';
  ctx.fillRect(x - 0.4 * zoom, y - h, 0.8 * zoom, h);

  // Pennant
  ctx.fillStyle = color;
  ctx.beginPath();
  ctx.moveTo(x + 0.4 * zoom, y - h);
  ctx.lineTo(x + 0.4 * zoom + w, y - h + w * 0.42);
  ctx.lineTo(x + 0.4 * zoom, y - h + w * 0.84);
  ctx.closePath();
  ctx.fill();
  ...
  if (isPlayer) {
    ...
    ctx.beginPath();
    ctx.arc(x, y - h, 1 * zoom, 0, Math.PI * 2);
    ctx.fill();
  }
}
...
function drawFarmDetails(ctx: CanvasRenderingContext2D, sx: number, sy: number, baseW: number, baseH: number, height: number, zoom: number, _palette: any, _building: Building, _time: number, state: GameState) {
  ...
  // Rows — thin filled strips, no outlines
  ctx.fillStyle = darken(cropColor, 0.72);
  for (let i = 0; i < 4; i++) {
    const x1 = sx - baseW * 0.4, y1 = sy + i * 2 * zoom;
    const x2 = sx + baseW * 0.3, y2 = sy + baseH * 0.3 + i * 1.5 * zoom;
    const t = 0.7 * zoom;
    ctx.beginPath();
    ctx.moveTo(x1, y1);
    ctx.lineTo(x2, y2);
...
function drawWarehouse(ctx: CanvasRenderingContext2D, sx: number, sy: number, baseW: number, baseH: number, height: number, zoom: number, palette: any, building: Building) {
  // Loading bay doors — a darker inset, distinguished by fill contrast alone
  for (let i = -1; i <= 1; i++) {
...
function drawEntity(ctx: CanvasRenderingContext2D, entity: MovingEntity, state: GameState, canvasW: number, canvasH: number, time: number) {
  ...
  switch (entity.type) {
    // Direction is a world-space cardinal (0=N, 1=E, 2=S, 3=W). The renderer
    // maps it onto the isometric screen axes so vehicles align with the drawn
    // road tiles — a horizontal world street reads as a 30° screen line.
    case 'car': drawCar(ctx, sx, sy, zoom, entity.color, screenAngleFor(entity.direction)); break;
    case 'truck': drawTruck(ctx, sx, sy, zoom, entity.color, screenAngleFor(entity.direction), time); break;
    case 'person': drawPerson(ctx, sx, sy, zoom, entity.color); break;
    case 'bus': drawBus(ctx, sx, sy, zoom, entity.color, screenAngleFor(entity.direction)); break;
    case 'freight_truck': drawTruck(ctx, sx, sy, zoom, '#dc2626', screenAngleFor(entity.direction), time); break;
  }
}

/**
 * Screen-space rotation for a vehicle whose world direction is `dir` (0=N,
 * 1=E, 2=S, 3=W). Under the classic 2:1 isometric projection the world x-axis
 * projects to ~30° on screen and the world y-axis to ~150°, so vehicles align
 * naturally with the diamond tiles they drive over.
 */
function screenAngleFor(dir: number): number {
  // atan2 of the projected direction vectors:
  //   world (1,0)  -> screen (TILE_W/2, TILE_H/2) → +30°
  //   world (0,1)  -> screen (-TILE_W/2, TILE_H/2) → +150°
  //   world (-1,0) -> screen (-TILE_W/2, -TILE_H/2) → -150°
  //   world (0,-1) -> screen (TILE_W/2, -TILE_H/2) → -30°
  const angles = [
    Math.atan2(-TILE_H / 2,  TILE_W / 2), // N (world dy = -1)
    Math.atan2( TILE_H / 2,  TILE_W / 2), // E (world dx = +1)
    Math.atan2( TILE_H / 2, -TILE_W / 2), // S (world dy = +1)
    Math.atan2(-TILE_H / 2, -TILE_W / 2), // W (world dx = -1)
  ];
  return angles[((dir % 4) + 4) % 4];
}
...
function drawCar(ctx: CanvasRenderingContext2D, x: number, y: number, zoom: number, color: string, dir: number) {
  ...
  ctx.save();
  ctx.translate(x, y);
  // `dir` is now a real screen-space angle in radians.
  ctx.rotate(dir);
  // Body
  ctx.fillStyle = color;
  ctx.beginPath();
...
function drawTruck(ctx: CanvasRenderingContext2D, x: number, y: number, zoom: number, color: string, dir: number, time: number) {
  ...
  ctx.fill();
  // Ribbed pattern — thin fill stripes, no outlines
  ctx.fillStyle = '#b8b8b8';
  for (let i = 0; i < 4; i++) {
    ctx.fillRect(-s * 1.3 + i * s * 0.5 - s * 0.02, -s * 0.6, s * 0.04, s * 1.2);
  }
  // Cabin
  ctx.fillStyle = color;
  ctx.beginPath();
