...
/** Tradeable non-equity assets: commodities, metals and crypto. */
...
export interface TradedAsset {
  ...
  /** Spot price in USD per unit. */
  price: number;
  /** Long-run anchor the price mean-reverts toward. */
  anchor: number;
  /** Annualised volatility, 0..1. Crypto is an order of magnitude above wheat. */
  volatility: number;
  /** Unit label shown in the UI (barrel, oz, bushel, BTC…). */
  unit: string;
  history: number[];
  /** Units held by the player. */
  playerHolding: number;
  /** Average cost basis of the player's position. */
  playerCostBasis: number;
}

/** A single equity or asset position with tax lot tracking. */
export interface Position {
  assetId: string;
  kind: 'equity' | 'asset';
  units: number;
  costBasis: number;
  /** Tick the lot was opened — decides short vs long-term capital gains. */
  openedTick: number;
}
...
export interface City {
  ...
  // ── Vital statistics (per 1,000 residents per year) ─────────────────────
  /** Crude birth rate. Falls with income and education (demographic transition). */
  birthRate: number;
  /** Crude death rate. Rises with median age, falls with healthcare access. */
  deathRate: number;
  /** Net migration rate — the swing factor in any real city's growth. */
  netMigrationRate: number;
  /** Births minus deaths, per 1,000. Cached for the UI. */
  naturalIncrease: number;
  /** People who wanted to move in but found no housing. Drives rent pressure. */
  suppressedMigration: number;
  /** Rolling annual totals so the player can read real numbers, not just rates. */
  birthsThisYear: number;
  deathsThisYear: number;
  migrationThisYear: number;
...
export interface Company {
  ...
  /** Shares actually in public float (outstanding minus treasury). */
  sharesFloat: number;
  /** Accumulated operating cash flow this period. */
  operatingCashFlow: number;
  /** Dividends paid last year ($). */
  dividendsPaid: number;
  /** Shares of its OWN stock held in treasury (from buybacks). */
  treasuryShares: number;
  /** Equity stakes this company holds in others: companyId → share count. */
  equityHoldings: Record<string, number>;
  /** Commodity / metal / crypto positions: assetId → units. */
  assetHoldings: Record<string, number>;
  /** Realised capital gains this tax year (drives the CGT bill). */
...
export interface Economy {
  ...
  /** Consumer Price Index, 100 = game start. The headline inflation gauge. */
  cpi: number;
  /** Short-term capital gains rate (held < 1 year). */
  shortTermCapitalGainsRate: number;
  /** Long-term capital gains rate (held >= 1 year). */
  longTermCapitalGainsRate: number;
  /** Central bank forward guidance shown to the market. */
  forwardGuidance: 'hawkish' | 'neutral' | 'dovish';
  /** Credibility of the inflation target, 0..1. Anchors expectations. */
  cbCredibility: number;
  /** Velocity of money — the V in MV = PY. */
...
export interface GameState {
  ...
  /** Commodities, precious metals, crypto and index funds. */
